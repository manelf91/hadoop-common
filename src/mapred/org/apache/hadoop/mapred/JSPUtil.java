...
import org.apache.hadoop.util.StringUtils;
 ...
 class JSPUtil {
	...
	/**
	 * Wraps the {@link JobInProgress} object and contains boolean for 'job view
	 * access' allowed or not. This class is only for usage by JSPs and
	 * Servlets.
	 */
	 ...
	 public String[] getSplitCount(String host, int port, String taskId){
		 String URL="http://" + host + ":" + port+ "/jsphelper?attemptid=" + taskId;
			URL url;
		      ...
		      try {
		         ...
		         rd.close();
		         String[] valueSet=result.split(":");
		         return valueSet;
		      } catch (IOException e) {
		          String[] resultSet=new String[1];
		          StringWriter errors = new StringWriter();
		    	  e.printStackTrace(new PrintWriter(errors));
		          resultSet[0]=errors.toString();
		          return resultSet;
		          //  e.printStackTrace();
	...
	Collections.sort(mapTasks,
	     ...
	     for (MapTaskStatistics t1: mapTasks)
         {
             ...
             {
                 if (t2.getMapStartTime() >= t1.getMapEndtime() && t2.getPreviousTask() == null && t1.getNextTask()==null)
                 {
                     t2.setPreviousTask(t1); 
                     t1.setNextTask(t2);
                 }
             }
         }
	     List<MapTaskStatistics> bars = new ArrayList<MapTaskStatistics>();

         for (MapTaskStatistics t1 : mapTasks)
         {
             if (t1.getPreviousTask() == null)
             {
                 bars.add(t1);
             }
         }
	...
	/**
	 * Validates if current user can view the job. If user is not authorized to
	 * view the job, this method will modify the response and forwards to an
	 * error page and returns Job with viewJobAccess flag set to false.
	 * 
	 ...
	 */
	public static JobWithViewAccessCheck checkAccessAndGetJob(
			final JobTracker jt, JobID jobid, HttpServletRequest request,
		...
		try {
			...
			String errMsg = " Interrupted while trying to access "
					+ jobid
					+ "<hr><a href=\"jobtracker.jsp\">Go back to JobTracker</a><br>";
			JSPUtil.setErrorAndForward(errMsg, request, response);
			myJob.setViewAccess(false);
	...
	public static long getMinStartTime(long minStartTime,long currentStartTime){
		if(minStartTime==0){
			return currentStartTime;
		}
		else{
			return minStartTime<currentStartTime?minStartTime:currentStartTime;
		}
	}
	
	public static long getMaxEndTime(long maxEndTime,long currentEndTime){
		if(maxEndTime==0){
			return currentEndTime;
		}
		else{
			return maxEndTime>currentEndTime?maxEndTime:currentEndTime;
		}
	}
	
	/**
	 * Sets error code SC_UNAUTHORIZED in response and forwards to error page
	 * which contains error message and a back link.
	 */
	...
	/**
	 * Method used to process the request from the job page based on the request
	 ...
	 * 
	 * @param label
	 *            display heading to be used in the job table.
	 * @param jobs
	 *            vector of jobs to be displayed in table.
	 * @param refresh
	 *            refresh interval to be used in jobdetails page.
	 * @param rowId
	...
	public static String generateJobTable(String label,
		...
		if (jobs.size() > 0) {
			...
			for (Iterator<JobInProgress> it = jobs.iterator(); it.hasNext(); ++rowId) {
				...
				String schedulingInfo = HtmlQuoting.quoteHtmlChars(job
						.getStatus().getSchedulingInfo());
				String diagnosticInfo = HtmlQuoting.quoteHtmlChars(job
						.getStatus().getFailureInfo());
				if (isModifiable) {
				...
				sb.append("<td id=\"job_"
						...
						+ ServletUtil.percentageGraph(
								status.reduceProgress() * 100, 80)
						+ "</td><td>" + desiredReduces + "</td><td> "
						+ completedReduces + "</td><td>" + schedulingInfo
	...
	public static String generateJobDropDown(Collection<JobInProgress> jobs,
			JobConf conf) {
	...
	public static String generateRetiredJobTable(JobTracker tracker, int rowId)
		...
		Iterator<RetireJobInfo> iterator = tracker.retireJobs.getAll()
				.descendingIterator();
		if (!iterator.hasNext()) {
			sb.append("<tr><td align=\"center\" colspan=\"8\"><i>none</i>"
					+ "</td></tr>\n");
			...
			for (int i = 0; i < 100 && iterator.hasNext(); i++) {
				...
				if (historyFile != null && !historyFile.equals("")) {
					try {
						historyFileUrl = URLEncoder.encode(
								info.getHistoryFile(), "UTF-8");
				...
				sb.append("<td id=\"job_"
						...
						(historyFileUrl == null ? "" : "<a href=\""
								+ JobHistoryServer
										.getHistoryUrlPrefix(tracker.conf)
								+ "/jobdetailshistory.jsp?logFile="
								+ historyFileUrl + "\">")
						+

						info.status.getJobId()
						+ "</a></td>"
						+
						...
						+ rowId
						+ "\">"
						+ info.status.getJobPriority().toString()
						+ "</td>"
						+ "<td id=\"user_"
						+ rowId
						+ "\">"
						+ HtmlQuoting.quoteHtmlChars(info.profile.getUser())
						+ "</td>"
						+ "<td id=\"name_"
						+ rowId
						+ "\">"
						+ HtmlQuoting.quoteHtmlChars(info.profile.getJobName())
						+ "</td>"
						+ "<td>"
						+ JobStatus.getJobRunState(info.status.getRunState())
						+ "</td>"
						+ "<td>"
						+ new Date(info.status.getStartTime())
						+ "</td>"
						+ "<td>"
						...
						"<td>"
						+ StringUtils.formatPercent(info.status.mapProgress(),
								2)
						+ ServletUtil.percentageGraph(
								info.status.mapProgress() * 100, 80)
						+ "</td>"
						+

						"<td>"
						+ StringUtils.formatPercent(
								info.status.reduceProgress(), 2)
						+ ServletUtil.percentageGraph(
						...
						"<td>"
						+ HtmlQuoting.quoteHtmlChars(info.status
								.getSchedulingInfo())
						+ "</td>"
						+ "<td>"
						+ HtmlQuoting.quoteHtmlChars(info.status
								.getFailureInfo()) + "</td></tr>\n");
				rowId++;
			}
		}
		sb.append("</table>\n");
	...
	/**
	 * Read a job-history log file and construct the corresponding
	 * {@link JobInfo} . Also cache the {@link JobInfo} for quick serving
	 * further requests.
	 * 
	 * @param logFile
	 * @param fs
	 ...
	 * @throws IOException
	 */
	static JobInfo getJobInfo(Path logFile, FileSystem fs, JobConf jobConf,
			ACLsManager acLsManager, String user) throws IOException {
		...
		synchronized (jobHistoryCache) {
			...
			if (jobInfo == null) {
				...
				LOG.info("Loading Job History file " + jobid
						+ ".   Cache size is " + jobHistoryCache.size());
				DefaultJobHistoryParser.parseJobTasks(
						logFile.toUri().getPath(), jobInfo, fs);
			}
			jobHistoryCache.put(jobid, jobInfo);
			int CACHE_SIZE = jobConf.getInt(
					"mapred.job.tracker.jobhistory.lru.cache.size", 5);
			if (jobHistoryCache.size() > CACHE_SIZE) {
				Iterator<Map.Entry<String, JobInfo>> it = jobHistoryCache
						.entrySet().iterator();
		...
		acLsManager.checkAccess(jobid, currentUser, jobInfo.getJobQueue(),
				Operation.VIEW_JOB_DETAILS, jobInfo.get(Keys.USER), jobInfo
						.getJobACLs().get(JobACL.VIEW_JOB));
	 ...
	 * @param fs
	 * @param logFile
	 * @return the job if authorization is disabled or if the authorization
	...
	static JobInfo checkAccessAndGetJobInfo(HttpServletRequest request,
			...
			final Path logFile) throws IOException, InterruptedException,
			ServletException {
		String jobid = getJobID(logFile.getName());
		String user = request.getRemoteUser();
		JobInfo job = null;
		if (user != null) {
			try {
				job = JSPUtil.getJobInfo(logFile, fs, jobConf, acLsManager,
						user);
			} catch (AccessControlException e) {
				String trackerAddress = jobConf
						.get("mapred.job.tracker.http.address");
				String errMsg = String
						.format("User %s failed to view %s!<br><br>%s"
								+ "<hr>"
	...
	private static void printJobACLsInternal(
			Map<JobACL, AccessControlList> jobAcls, JspWriter out)
			throws IOException {
